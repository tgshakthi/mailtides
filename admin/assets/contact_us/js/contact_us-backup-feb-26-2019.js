...
var field_id_rows = $('#field_id_rows').val();
var label_names = (label_names != undefined) ? label_names.split(',') : new Array();
var field_id_rows = (field_id_rows != undefined) ? field_id_rows.split(',') : new Array();

for (var l = 0; l < label_names.length; l++) {
	$('#' + label_names[l] + 'ff').hide();
	$("#select_column option[value='" + field_id_rows[l] + '/' + label_names[l] + "']").hide();
}
...
function remove_from_fields(id) {
	var remove_id = $('#remove_form_fields_id_' + id).val();
	if (remove_id == undefined) {
		$("#form_id_" + id).remove();
	} else if (remove_id.length != 0) {
		...
		if (!conf) {
			...
			$.ajax({
				type: "POST",
				url: 'contact_us/contact_form_field_remove',
				data: {
					remove_id: remove_id
				},
...
function add_morerowcolumn1(row_id) {
	...
	var field_name = field_name.split('/');
	var field_id = field_name[0];
	var field_name = field_name[1];
	var field_name1 = field_name.replace(/_/g, " ");
	var field_name1 = field_name1[0].toUpperCase() + field_name1.slice(1);
	$("#select_column option[value='" + field_id + '/' + field_name + "']").hide();
	...
	var data = '<li class="' + size + '" id="form_row_column_' + row_id + c + '"><div><label>' + field_name1 + '</label><button type="button" onclick="remove_form_row_column(' + row_id + c + ',' + row_id + ')"><input type="hidden" name="select_column_' + row_id + '[]" id="select_column_' + row_id + '" value="' + c + '"><input type="hidden" name="select_row_column_' + row_id + '[]" id="select_row_column_' + row_id + '" value="' + row_id + c + '"><input type="hidden" name="row_column_field_id' + row_id + c + '[]" id="row_column_field_id' + row_id + c + '" value="' + field_id + '"><input type="hidden" name="row_column_field_name' + row_id + c + '[]" id="row_column_field_name' + row_id + c + '" value="' + field_name + '"><i class="fa fa-times-circle" aria-hidden="true"></i></button></div></li>';
	...
	if (old_row_column_no != '') {
		var old_row_column_no = old_row_column_no + ',' + field_id;
	} else {
		var old_row_column_no = field_id;
	}
...
function remove_form_row_column(id, column_id) {
	...
	var field_id = $("#row_column_field_id" + id).val();
	...
	$("#select_column option[value='" + field_id + '/' + field_name + "']").show();
