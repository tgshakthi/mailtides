...
if ($("#mybarChart").length) {
    ...
    new Chart(f, {
        type: "bar",
        data: {
            labels: ["Sent", "Opened", "Unopened", "Link Opened", "Comments Posted", "Comments Not Posted"],
            datasets: [{
                backgroundColor: ["#26B99A", "#EE82EE", "#DA70D6", "#006600", "#CC0066", "#000099"],
                data: [mailSent, opened, notOpened, txgidocsReviews, commentsPosted, commentsNotPosted]
            }]
        },
        options: {
            legend: {
...
$(document).ready(function () {
    ...
    // Table Two
    $('#datatable-campaign-users>thead>tr').clone(true).appendTo('#datatable-campaign-users thead');
    ...
    if (campaignId.length > 0) {
        $.ajax({
            ...
            success: function (data) {
                if (data != 0) {
                    if (data == 1) {
                        var img = '<img class="preview-template-img" src="' + imageUrl + 'images/txgidocs/mail-template/digestive-and-liver-disease-consultants-template.png">';
                        $('#preview-template-modal').modal('show');
                        $('#modal-body-img').html(img);
                    }

                    if (data == 2) {
                        var img = '<img class="preview-template-img" src="' + imageUrl + 'images/txgidocs/mail-template/google-template.png">';
                        $('#preview-template-modal').modal('show');
                        $('#modal-body-img').html(img);
                    }

                    if (data == 3) {
                        var img = '<img class="preview-template-img" src="' + imageUrl + 'images/txgidocs/mail-template/facebook-template.png">';
                        $('#preview-template-modal').modal('show');
                        $('#modal-body-img').html(img);
                    }
...
$('#filter-data-import').click(function () {
    var values = $("input[name='row_sort_order[]']").map(function () {
        return $(this).val();
    }).get();
