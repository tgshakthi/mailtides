	...
	$('#filter-data-import').click(function () {
		...
		if (campaignName.length > 0) {
			if (values.length > 0) {
				$.ajax({
					method: 'POST',
					url: baseUrl + 'email_blasts/import_filter_data',
					data: {
					...
					success: function (data) {
						if (data.length > 0) {
							...
							window.location.href = baseUrl + 'email_blasts/add_edit_campaign';
						}
					}
				});
			} else {
	...
	function onFinishCallback() {
		...
		if (campaignName.length > 0 && emailTemplate.length > 0) {
			if (values.length > 0) {
				$.ajax({
					method: 'POST',
					url: baseUrl + 'email_blasts/insert_campaign_data',
					data: {
						campaign_category_id: campaign_category_id,
						campaign_id: campaignId,
						campaign_name: campaignName,
					...
					success: function (data) {
						window.location.href = baseUrl + 'email_blasts/campaign';
					}
				});
			}
...
if ($('#mybarChart').length) {
	...
	function campaign(e) {
		...
		$.ajax({
			method: 'POST',
			url: baseUrl + 'email_blasts/graphical_campaign_id',
			data: {
				campaign_id: e,
			...
			success: function (data) {
				...
				if (campaignData.type == 'email') {
					...
					$('#title').show();
					$('#title-report').show();
					var link_not_opened = (campaignData.not_opened - campaignData.link_open);
...
function campaign_type(value) {
	...
	$.ajax({
		method: 'POST',
		url: baseUrl + 'email_blasts/get_campaign_by_type',
		data: {
			campaign_type: value
		},
		cache: false,
		success: function (data) {
			if (data == "0") {
				...
				$('#campaign_name_data').html('<option value="">Select Campaign</option>' + data);
			}
		}
	});
}
...
$('#preview_template').click(function () {
	...
	if (campaignId.length > 0) {
		$.ajax({
			method: 'POST',
			url: baseUrl + 'email_blasts/get_campaign_template',
			data: {
				campaign_id: campaignId
			},
			success: function (data) {
...
$(document).ready(function () {
	...
	$('#btn_ok').click(function () {
		...
		$.ajax({
			type: 'POST',
			url: siteUrl + 'email_blasts/remove_email_template_image',
			data: {
				id: id
			},
			cache: false,
...
$('#campaign-name').blur(function () {
	...
	if (Name.length != 0) {
		$.ajax({
			type: 'POST',
			url: base_url + 'email_blasts/check_campaign_name',
			data: {
				campaign_name: Name
			},
			success: function (data) {
...
$('#campaign_type').blur(function () {
	...
	if (Name.length != 0) {
		$.ajax({
			type: 'POST',
			url: base_url + 'email_blasts/check_campaign_type_name',
			data: {
				campaign_name: Name,
...
if ($('#mybarChart_type').length) {
	...
	function campaign_types(m) {
		...
		$.ajax({
			method: 'POST',
			url: baseUrl + 'email_blasts/graphical_campaign_type',
			data: {
...
$('#phone_number').blur(function () {
	...
	$.ajax({
		type: 'POST',
		url: base_url + 'email_blasts/check_patient_phone_number',
		data: {
