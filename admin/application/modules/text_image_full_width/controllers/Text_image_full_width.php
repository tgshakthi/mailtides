...
class Text_image_full_width extends MX_Controller
{
    ...
    function text_image_full_width_index($page_id)
    {
		...
		$data['heading'] = 'Text Image Full width';
		$data['title']	 = "Text Image Full Width| Administrator";
		$this->load->view('template/meta_head', $data);
		$this->load->view('text_image_full_width_header');
    ...
    function insert_update_text_image_full_width_background()
    {
        $page_id = $this->input->post('page_id');
        $this->Text_image_full_width_model->insert_update_text_image_background($page_id);
        redirect('text_image_full_width/text_image_full_width_index/' . $page_id);
    }
	...
	function get_table($page_id)
	{
		...
		$text_images	= $this->Text_image_full_width_model->get_text_image_full_width($page_id);
		if (isset($text_images) && $text_images != "")
		{
			foreach ($text_images as $text_image)
			{
				$anchor_edit = anchor(
					'text_image_full_width/add_edit_text_image_full_width/'.$page_id.'/'.$text_image->id,
				...
				$anchor_delete = anchor(
					...
					array(
						...
						'onclick' => 'return delete_record('.$text_image->id.', \''.base_url('text_image_full_width/delete_text_image_full_width/'.$page_id).'\')'
					)
				);
	...
	function update_sort_order()
	{
		...
		$this->Text_image_full_width_model->update_sort_order($page_id, $row_sort_order);
	}

	// Add & Edit Text Image
	function add_edit_text_image_full_width($page_id, $id = null)
	{
		$data['page_id']	= $page_id;
		...
		if ($id != null)
		{
			...
			$text_image = $this->Text_image_full_width_model->get_text_image_full_width_by_id($page_id, $id);

			$data['text_image_full_width_id']	= $text_image[0]->id;
			...
			$data['background'] = $text_image[0]->background;
		...
		if (!empty($data['background'])) :
			$text_image_full_width_bg = json_decode($data['background']);
			$data['component_background'] = $text_image_full_width_bg->component_background;
			$data['text_image_full_width_background'] = $text_image_full_width_bg->text_image_full_width_background;
		else :
			$data['component_background'] = "";
			$data['text_image_full_width_background'] = "";
		endif;
		
		$data['httpUrl']	= $this->admin_header->host_url();
		$data['ImageUrl']	= $this->admin_header->image_url();
		$data['website_folder_name'] = $this->admin_header->website_folder_name();
		$data['title']	= ($id != null) ? 'Edit Text Image full width' : 'Add Text Image full Width'.' | Administrator';
		$data['heading']	= (($id != null) ? 'Edit' : 'Add').' Text Image full Width';
		$this->load->view('template/meta_head', $data);
		$this->load->view('text_image_full_width_header');
		$this->admin_header->index();
		$this->load->view('add_edit_text_image_full_width', $data);
		$this->load->view('template/footer_content');
    	$this->load->view('script');
		$this->load->view('template/footer');
	}
	...
	function insert_update_text_image_full_width()
	{
	
		$page_id  = $this->input->post('page-id');
		$text_image_id  = $this->input->post('text-image-full-width-id');
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			{
				redirect('text_image_full_width/add_edit_text_image_full_width/'.$page_id);
			}
			else
			{
				redirect('text_image_full_width/add_edit_text_image_full_width/'.$page_id.'/'.$text_image_id);
			}
		}
		else
		{
			if (empty($text_image_id))
			{
				$insert_id	= $this->Text_image_full_width_model->insert_update_text_image_full_width_data($page_id);
				$this->session->set_flashdata('success', 'Text Image Full Width successfully Created');
				if (isset($continue) && $continue === "Add & Continue")
				{
					$url = 'text_image_full_width/add_edit_text_image_full_width/'.$page_id.'/'.$insert_id;
				}
				else
				{
					$url = 'text_image_full_width/text_image_full_width_index/'.$page_id;
				}
			}
			else
			{
				$this->Text_image_full_width_model->insert_update_text_image_full_width_data($page_id, $text_image_id);
				$this->session->set_flashdata('success', 'Text Image Full Width Successfully Updated.');
				if (isset($continue) && $continue === "Update & Continue")
				{
					$url = 'text_image_full_width/add_edit_text_image_full_width/'.$page_id.'/'.$text_image_id;
				}
				else
				{
					$url = 'text_image_full_width/text_image_full_width_index/'.$page_id;
	...
	{
		$this->Text_image_full_width_model->delete_text_image_full_width($page_id);
		$this->session->set_flashdata('success', 'Text Image Full Width  Successfully Deleted.');
		redirect('text_image_full_width/text_image_full_width_index/'.$page_id);
	}
	...
	function delete_multiple_text_image_full_width()
	{
		...
		{
			$this->Text_image_full_width_model->delete_multiple_text_image_full_width();
			$this->session->set_flashdata('success', 'Successfully Deleted');
			redirect('text_imag_full_widthe/text_image_full_width_index/'.$page_id);
		}
	}
	...
	{
		$this->Text_image_full_width_model->remove_image();
		echo '1';
	}
